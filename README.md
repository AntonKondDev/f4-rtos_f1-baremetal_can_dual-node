...
## Возможности проекта  
- Двунаправленный обмен по CAN между F4 и F1.  
- F4 каждые 500 мс отправляет PING-кадр (ID 0x100).  
- F1 принимает PING, отправляет PONG-кадр (ID 0x101).  
- Индикация событий светодиодами на обеих платах.  
- Использование задач FreeRTOS и очередей (на F4).  
- Обработка CAN через прерывания FIFO0 (на F1).  
- Корректный bit-timing 500 кбит/с при разных частотах шин APB1.  

## Скриншоты и видео
- [Данные с логического анализатора](https://github.com/user-attachments/assets/07c977aa-b1a9-41cd-843f-f6c2fb7fee45)
...
- Настройка PA11 (RX) и PA12 (TX) под CAN1 (через AFIO remap при необходимости).
